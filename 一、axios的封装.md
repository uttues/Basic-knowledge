axios的封装和api接口的统一管理，其实主要目的就是在帮助我们**简化代码**和利于**后期的更新维护**。

[axios文档](https://www.npmjs.com/package/axios)

## 一、axios的封装

在vue项目中，和后台交互获取数据这块，我们通常使用的是axios库，它是**基于promise的http库**，**可运行在浏览器端和node.js中**。他有很多优秀的特性，例如拦截请求和响应、取消请求、转换json、客户端防御XSRF等。
...
项目环境可能有**开发环境、测试环境和生产环境**，可以通过**node的环境变量**来匹配我们的**默认的接口url前缀**。
...
在发送请求前可以进行一个请求的拦截，从而进行我们想要的操作。

作用：比如，有些请求是需要用户登录之后才能访问的，或者post请求的时候，我们需要序列化我们提交的数据
...
> token
>
> * 在登录完成之后，将用户的token通过**localStorage或者cookie**存在本地，用户每次在进入页面的时候（即在main.js中），会首先**从本地存储中读取token**（前端鉴权？），如果token存在说明用户已经登陆过，则**更新vuex中的token状态**。
> * 之后每次请求接口时，都会**在请求的header中携带token**，后台人员就可以根据你携带的token来判断你的登录是否过期，如果没有携带，则说明没有登录过。
>   * 每个请求都会携带token，对于不需要用户登录就能访问的页面，后端设置不判断



### 响应的拦截

在拿到服务器返回给我们的数据之前可以将响应拦截下来，对他进行一些处理。

例如上面的思想：如果后台返回的状态码是200，则正常返回数据，否则的根据错误的状态码类型进行一些错误处理，其实这里主要就是**进行了错误的统一处理和没登录或登录过期后调整登录页的一个操作**。
...
export function get(url, params){    
    return new Promise((resolve, reject) =>{   
        // get的第二个参数是一个对象，对象中的params是参数对象
        axios.get(url, {            
            params: params        
        }).then(res => {
            resolve(res.data);
...
**post方法：**原理同get基本一样，但是要注意的是，post方法必须要使用对提交从参数对象进行序列化的操作通过node的qs模块来序列化参数。如果没有序列化操作，后台是拿不到你提交的数据的。
...
把api统一集中起来，如果后期需要修改接口，我们就直接在api.js中找到对应的修改就好了，而不用去每一个页面查找我们的接口然后再修改会很麻烦。关键是，万一修改的量比较大，就规格gg了。还有就是如果直接在我们的业务代码修改接口，一不小心还容易动到我们的业务代码造成不必要的麻烦。
...
这是app.vue，这里简单演示一下断网。在http.js中介绍了，我们会**在断网的时候，来更新vue中network的状态**，那么这里我们根据network的状态来判断是否需要加载这个断网组件。断网情况下，加载断网组件，不加载对应页面的组件。当点击刷新的时候，我们通过跳转refesh页面然后立即返回的方式来实现重新获取数据的操作。因此我们需要新建一个refresh.vue页面，并在其`beforeRouteEnter`钩子中再返回当前页面。
